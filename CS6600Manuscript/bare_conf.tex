...
For a Kalman filter, its posteriors are Gaussian if the following three properties hold\\
...
Figure \ref{EssSlam} shows EKF SLAM in operation. The robot moves through the environment and simultaneously estimates both its own location and those of the landmarks. Due to uncertainties in robot movement and sensor measurements, the true locations are never known, but an estimate consisting of a mean value and covariance is computed. The estimated mean value may be different from the true one, but all the estimates are correlated with each other. As the robot moves through the environment, and observes the same landmark multiple times, the correlations between landmarks increases. Landmarks that the robot can observe together, end up being more correlated with each other. This is shown in figure \ref{SpringSlam} where the increased correlation is shown by thicker red springs. All landmarks locations are correlated with each other and the robot, with the landmarks that can be observed at the same time being more strongly correlated and connected by thicker springs.\\
...
The movement Jacobian can be found by computing the partial derivative of the motion equation with respect to each component of the robot pose. This derivation is given in section \ref{sssec:num3}. This Jacobian was used in line $3$ of the EKF algorithm.\\
...
The robot is able to move around and navigate through an obstacle course. However, as described above, the prediction step of the EKF SLAM process was not implemented.\\
...
The movement Jacobian represented by $G_t$ in line $3$ of the EKF algorithm can be obtained by finding the partial derivative of the movement function $g$ \cite{stachniss}. In the case of the move forward, the Jacobian is given by
